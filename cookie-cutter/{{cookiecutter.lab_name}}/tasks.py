...
namespace = Collection("{{ cookiecutter.lab_name }}")
namespace.configure(
    {
        "{{ cookiecutter.lab_name }}": {
            "nautobot_ver": "1.4.2",
            "project_name": "{{ cookiecutter.lab_name }}",
            "python_ver": "3.8",
            "local": False,
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.{{ cookiecutter.lab_name }}.compose_http_timeout,
        "NAUTOBOT_VER": context.{{ cookiecutter.lab_name }}.nautobot_ver,
        "PYTHON_VER": context.{{ cookiecutter.lab_name }}.python_ver,
    }
    compose_command = f'docker-compose --project-name {context.{{ cookiecutter.lab_name }}.project_name} --project-directory "{context.{{ cookiecutter.lab_name }}.compose_dir}"'
    for compose_file in context.{{ cookiecutter.lab_name }}.compose_files:
        compose_file_path = os.path.join(context.{{ cookiecutter.lab_name }}.compose_dir, compose_file)
...
def launch_browser(context, url=None):
    try:
        if url is None:
            result = context.run(f"docker port {context.{{ cookiecutter.lab_name }}.project_name}_nautobot_1", hide=True, warn=True)
...
def run_command(context, command, container="nautobot", **kwargs):
    ...
    if is_truthy(context.{{ cookiecutter.lab_name }}.local):
