...
namespace = Collection("git_repository")
namespace.configure(
    {
        "git_repository": {
            "nautobot_ver": "1.4.2",
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.git_repository.compose_http_timeout,
        "NAUTOBOT_VER": context.git_repository.nautobot_ver,
        "PYTHON_VER": context.git_repository.python_ver,
    }
    compose_command = f'docker-compose --project-name {context.git_repository.project_name} --project-directory "{context.git_repository.compose_dir}"'
    for compose_file in context.git_repository.compose_files:
        compose_file_path = os.path.join(context.git_repository.compose_dir, compose_file)
...
def launch_browser(context, url=None):
    try:
        if url is None:
            result = context.run(f"docker port {context.git_repository.project_name}_nautobot_1", hide=True, warn=True)
...
def run_command(context, command, container="nautobot", **kwargs):
    ...
    if is_truthy(context.git_repository.local):
